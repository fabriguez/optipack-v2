...
//
// Source de verite unique : ce fichier vit dans `src/` (compile dans dist) afin
// que le seed (prisma/seed.ts) ET le self-heal runtime (PermissionSeedService,
// appele au boot dans src/index.ts) partagent le meme catalogue. `prisma/` etant
// exclu du build tsc, aucune donnee de seed ne doit y rester.
...
export const PERMISSION_CATALOG: Array<{ key: string; label: string; category: string; description?: string }> = [
  ...
  { key: 'expense.update', label: 'Modifier une depense', category: 'depense', description: 'Modifier une depense existante (nature, montant, piece justificative) avant son reglement.' },
  { key: 'expense.delete', label: 'Supprimer une depense', category: 'depense', description: 'Supprimer une depense enregistree par erreur. A reserver aux responsables finances.' },
  ...
  { key: 'notification.manage', label: 'Gerer les notifications (relancer un envoi)', category: 'notification', description: 'Relancer manuellement l\'envoi d\'une notification en echec et piloter la file d\'envoi.' },
...
export const POSITION_CATALOG: Array<{
...
}> = [
  {
    ...
    permissions: [
      ...
      'accounting.read',
      'expense.read', 'expense.create', 'expense.update', 'expense.delete', 'expense.approve', 'expense.pay', 'charge.manage',
