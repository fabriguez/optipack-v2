...
import { createFundTransferSchema, type CreateFundTransferInput } from '@optipack/shared';
...
export function FundTransferFormDialog({ open, onClose }: Props) {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<CreateFundTransferInput>({
  ...
  return (
    <AppDialog open={open} onClose={onClose} title="Nouveau transfert de fonds" size="md">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <AppSelect label="Agence source" {...register('sourceAgencyId')} error={errors.sourceAgencyId?.message}
