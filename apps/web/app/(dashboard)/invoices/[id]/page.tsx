...
export default function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Annulation d'un paiement : administrateurs uniquement, en plus de la
  // permission payment.void (miroir de la route API POST /payments/:id/void).
      ...
      render: (row: any) => (
        <RowActions
          actions={[
            ...
            ...(canVoidPayment ? [{ label: 'Annuler', icon: <XCircle className="h-4 w-4" />, onClick: () => router.push(`/payments/${row.id}`), variant: 'destructive' as const, disabled: row.isVoided }] : []),
          ]}
        />
      ),
    },
