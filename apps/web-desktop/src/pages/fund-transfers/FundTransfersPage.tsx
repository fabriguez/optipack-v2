    ...
    { key: 'createdAt', label: 'Date', render: (row: any) => formatDateTime(row.createdAt) },
    {
      ...
      render: (row: any) => (
        <RowActions actions={[
          ...
          { label: 'Confirmer', icon: <CheckCircle className="h-4 w-4" />, onClick: () => navigate(`/fund-transfers/${row.id}`), disabled: row.status !== 'PENDING' },
          { label: 'Annuler', icon: <Ban className="h-4 w-4" />, onClick: () => navigate(`/fund-transfers/${row.id}`), variant: 'destructive', disabled: row.isVoided || row.status === 'VOIDED' },
        ]} />
      ),
    },
  ];
