...
export default function FundTransfersPage() {
  ...
  const columns = [
    { key: 'reference', label: 'Reference', render: (row: any) => <Link to={`/fund-transfers/${row.id}`} className="font-mono text-xs text-primary-700 font-medium hover:underline" onClick={(e) => e.stopPropagation()}>{row.reference}</Link> },
    ...
    { key: 'createdAt', label: 'Date', render: (row: any) => formatDateTime(row.createdAt) },
    {
      ...
      render: (row: any) => (
        <RowActions actions={[
          { label: 'Voir details', icon: <Eye className="h-4 w-4" />, onClick: () => navigate(`/fund-transfers/${row.id}`) },
          ...(canConfirm ? [{ label: 'Confirmer', icon: <CheckCircle className="h-4 w-4" />, onClick: () => navigate(`/fund-transfers/${row.id}`), disabled: row.status !== 'PENDING' }] : []),
          ...(canVoid ? [{ label: 'Annuler', icon: <Ban className="h-4 w-4" />, onClick: () => navigate(`/fund-transfers/${row.id}`), variant: 'destructive' as const, disabled: row.isVoided || row.status === 'VOIDED' }] : []),
        ]} />
      ),
    },
  ];
