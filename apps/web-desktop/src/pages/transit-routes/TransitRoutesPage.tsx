...
/** Formatte la valeur ajoutee : "+2 000 FCFA" (AMOUNT), "+10%" (PERCENT) ou "-". */
...
export default function TransitRoutesPage() {
  ...
  const columns = [
    { key: 'name', label: 'Nom', render: (row: any) => <Link to={`/transit-routes/${row.id}`} className="text-primary-700 font-medium hover:underline" onClick={(e) => e.stopPropagation()}>{row.name}</Link> },
    ...
    { key: 'isActive', label: 'Statut', render: (row: any) => <AppBadge variant={row.isActive ? 'success' : 'error'}>{row.isActive ? 'Actif' : 'Inactif'}</AppBadge> },
    {
      ...
      render: (row: any) => (
        <RowActions actions={[
          { label: 'Voir details', icon: <Eye className="h-4 w-4" />, onClick: () => navigate(`/transit-routes/${row.id}`) },
          { label: 'Modifier', icon: <Edit className="h-4 w-4" />, onClick: () => navigate(`/transit-routes/${row.id}`) },
        ]} />
      ),
    },
  ];
