...
import { WarehouseFormDialog } from '@/pages/warehouses/WarehouseFormDialog';
import { ClientFormDialog } from '@/pages/clients/ClientFormDialog';
import { EmployeeFormDialog } from '@/pages/employees/EmployeeFormDialog';
import { PayEmployeeDialog } from '@/pages/employees/PayEmployeeDialog';
import { SalaryDeductionDialog } from '@/pages/employees/SalaryDeductionDialog';
...
export default function AgencyDetailPage() {
  ...
  const warehouseColumns = [
    { key: 'name', label: 'Nom', render: (row: any) => <Link to={`/warehouses/${row.id}`} className="font-medium text-primary-700 hover:underline">{row.name}</Link> },
    { key: 'location', label: 'Emplacement' },
    { key: '_count', label: 'Colis', render: (row: any) => row._count?.parcels ?? 0 },
    { key: 'actions', label: '', className: 'w-10', render: (row: any) => <RowActions actions={[{ label: 'Voir', icon: <Eye className="h-4 w-4" />, onClick: () => navigate(`/warehouses/${row.id}`) }]} /> },
  ];

  const clientColumns = [
    { key: 'fullName', label: 'Nom', render: (row: any) => <Link to={`/clients/${row.id}`} className="font-medium text-primary-700 hover:underline">{row.fullName}</Link> },
    { key: 'phone', label: 'Telephone' },
    { key: 'loyaltyTier', label: 'Fidelite', render: (row: any) => <AppBadge>{row.loyaltyTier}</AppBadge> },
    { key: 'actions', label: '', className: 'w-10', render: (row: any) => <RowActions actions={[{ label: 'Voir', icon: <Eye className="h-4 w-4" />, onClick: () => navigate(`/clients/${row.id}`) }]} /> },
  ];
  ...
  const disbursementColumns = [
    { key: 'reference', label: 'Reference', render: (row: any) => <Link to={`/disbursements/${row.id}`} className="font-mono text-xs text-primary-700 hover:underline">{row.reference}</Link> },
  ...
  const paymentColumns = [
    { key: 'reference', label: 'Reference', render: (row: any) => <Link to={`/payments/${row.id}`} className="font-mono text-xs text-primary-700 hover:underline">{row.reference}</Link> },
