  ...
  const columns = [
    {
      ...
      render: (row: any) => (
        <div className="flex items-center gap-2.5">
          ...
          <Link to={`/invoices/${row.id}`} className="font-mono text-xs font-bold text-primary-700 hover:underline" onClick={(e) => e.stopPropagation()}>{row.reference}</Link>
        </div>
      ),
    },
    {
    ...
    { key: 'issuedAt', label: 'Date', render: (row: any) => <span className="text-xs text-gray-500">{formatDate(row.issuedAt)}</span> },
    {
      ...
      render: (row: any) => (
        <RowActions actions={[
          ...
          ...(canRecordPayment ? [{ label: 'Enregistrer paiement', icon: <CreditCard className="h-4 w-4" />, onClick: () => navigate(`/payments?invoiceId=${row.id}`), disabled: (row.effectiveStatus ?? row.status) === 'PAID' }] : []),
        ]} />
      ),
    },
  ];
