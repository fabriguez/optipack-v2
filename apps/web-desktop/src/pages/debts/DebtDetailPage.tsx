...
export default function DebtDetailPage() {
  ...
  const TierBlock = () => {
    switch (debt.type) {
      case 'CLIENT':
        return debt.client ? (
          <Link to={`/clients/${debt.client.id}`} className="text-sm font-medium text-primary-700 hover:underline">
      ...
      case 'EMPLOYEE':
        return debt.employee ? (
          <Link to={`/employees/${debt.employee.id}`} className="text-sm font-medium text-primary-700 hover:underline">
          ...
          <AppCard>
            ...
            {debt.invoice && (
              <p className="mt-2 text-xs">
                Facture liee :{' '}
                <Link to={`/invoices/${debt.invoice.id}`} className="font-mono text-primary-700 hover:underline">
            ...
            {debt.parcel && (
              <p className="mt-2 text-xs">
                Colis lie :{' '}
                <Link to={`/parcels/${debt.parcel.id}`} className="font-mono text-primary-700 hover:underline">
