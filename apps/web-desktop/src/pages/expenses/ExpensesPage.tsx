...
export default function ExpensesPage() {
  ...
  const columns = [
    { key: 'title', label: 'Titre', render: (row: any) => <Link to={`/expenses/${row.id}`} className="text-primary-700 font-medium hover:underline" onClick={(e) => e.stopPropagation()}>{row.title}</Link> },
    { key: 'reason', label: 'Motif' },
    ...
    { key: 'createdAt', label: 'Date', render: (row: any) => formatDate(row.createdAt) },
    {
      ...
      render: (row: any) => (
        <RowActions actions={[
          { label: 'Voir details', icon: <Eye className="h-4 w-4" />, onClick: () => navigate(`/expenses/${row.id}`) },
        ]} />
      ),
    },
  ];
