  ...
  const columns = [
    {
      ...
      render: (row: any) => (
        <div className="flex items-center gap-2.5">
          ...
          <Link to={`/payments/${row.id}`} className="font-mono text-xs font-bold text-primary-700 hover:underline" onClick={(e) => e.stopPropagation()}>{row.reference}</Link>
        </div>
      ),
    },
