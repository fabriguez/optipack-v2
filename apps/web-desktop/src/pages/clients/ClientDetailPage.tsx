...
import { ArrowLeft, User, Package, CreditCard, Star, FileText, AlertTriangle, Plus, Eye } from 'lucide-react';
  ...
  const parcelColumns = [
    {
      ...
      label: 'Tracking',
      render: (row: any) => (
        <Link to={`/parcels/${row.id}`} className="font-mono text-xs text-primary-700 font-bold hover:underline" onClick={(e) => e.stopPropagation()}>
          {row.trackingNumber}
        </Link>
      ),
    },
  ...
  const invoiceColumns = [
    {
      ...
      label: 'Reference',
      render: (row: any) => (
        <Link to={`/invoices/${row.id}`} className="font-mono text-xs text-primary-700 font-bold hover:underline" onClick={(e) => e.stopPropagation()}>
          {row.reference}
        </Link>
      ),
    },
    ...
    { key: 'issuedAt', label: 'Date', render: (row: any) => formatDate(row.issuedAt) },
    {
      ...
      render: (row: any) => (
        <RowActions
          actions={[
            { label: 'Voir', icon: <Eye className="h-4 w-4" />, onClick: () => navigate(`/invoices/${row.id}`) },
            { label: 'Payer', icon: <CreditCard className="h-4 w-4" />, onClick: () => navigate(`/payments?invoiceId=${row.id}`), disabled: row.status === 'PAID' },
          ]}
        />
      ),
    },
  ...
  const debtColumns = [
    {
      ...
      label: 'Description',
      render: (row: any) => (
        <Link to={`/debts/${row.id}`} className="text-sm text-primary-700 font-medium hover:underline" onClick={(e) => e.stopPropagation()}>
          {row.description}
        </Link>
      ),
    },
    ...
    },
    {
      ...
      render: (row: any) => (
        <RowActions
          actions={[
            { label: 'Voir', icon: <Eye className="h-4 w-4" />, onClick: () => navigate(`/debts/${row.id}`) },
          ]}
        />
      ),
    },
  ...
  const debtsTab = (
    <AppCard>
      ...
      <AppDataTable
        columns={debtColumns}
        data={debtsData?.data || []}
        onRowClick={(row) => navigate(`/debts/${row.id}`)}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3">
              ...
              {client.employee && (
                <Link to={`/employees/${client.employee.id}`} className="inline-flex">
                  <AppBadge variant="info">Employe →</AppBadge>
                </Link>
              )}
              {client.carrier && (
                <Link to={`/carriers/${client.carrier.id}`} className="inline-flex">
