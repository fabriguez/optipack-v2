...
export default function WarehousesPage() {
  ...
  const columns = [
    { key: 'name', label: 'Nom', render: (row: any) => <Link to={`/warehouses/${row.id}`} className="text-primary-700 font-medium hover:underline" onClick={(e) => e.stopPropagation()}>{row.name}</Link> },
    { key: 'location', label: 'Emplacement' },
    ...
    { key: 'isActive', label: 'Statut', render: (row: any) => <AppBadge variant={row.isActive ? 'success' : 'error'}>{row.isActive ? 'Actif' : 'Inactif'}</AppBadge> },
    {
      ...
      render: (row: any) => (
        <RowActions actions={[
          { label: 'Voir details', icon: <Eye className="h-4 w-4" />, onClick: () => navigate(`/warehouses/${row.id}`) },
        ]} />
      ),
    },
  ];
