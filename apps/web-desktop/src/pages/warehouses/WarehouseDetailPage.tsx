...
export default function WarehouseDetailPage() {
  ...
  const handleStartInventory = async () => {
    try {
      ...
      navigate(`/warehouses/${id}/inventory/${res.data.data.id}`);
    } catch (e: any) {
    ...
    <PageTransition>
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          {warehouse.agency && (
            <Link to={`/agencies/${warehouse.agency.id}`} title={warehouse.agency.name}>
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              ...
              {warehouse.agency && (
                <>
                  {' '}&mdash;{' '}
                  <Link to={`/agencies/${warehouse.agency.id}`} className="text-primary-600 hover:underline">
                        ...
                        {items.map((inv: any) => (
                          <tr key={inv.id} className="hover:bg-gray-50">
                            ...
                            <td className="py-2 text-right">
                              <Link to={`/warehouses/${id}/inventory/${inv.id}`}>
        ...
        <AppCard>
          ...
          {parcelView === 'groups' ? (
            <AppDataTable
              ...
              onRowClick={(row) => navigate(`/parcel-groups/${row.id}`)}
            />
          ) : (
          <>
