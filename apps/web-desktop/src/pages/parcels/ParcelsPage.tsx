  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: any) => (
        <div className="flex items-center gap-2.5">
          ...
          <Link to={`/parcels/${row.id}`} className="font-mono text-xs font-bold text-primary-700 hover:underline" onClick={(e) => e.stopPropagation()}>{row.trackingNumber}</Link>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (row: any) =>
        row.warehouse ? (
          <Link
            to={`/warehouses/${row.warehouse.id}`}
