...
export default function DisbursementsPage() {
  ...
  const columns = [
    { key: 'reference', label: 'Reference', render: (row: any) => <Link to={`/disbursements/${row.id}`} className="font-mono text-xs text-primary-700 font-medium hover:underline" onClick={(e) => e.stopPropagation()}>{row.reference}</Link> },
    { key: 'reason', label: 'Motif' },
    { key: 'orderer', label: 'Ordonnateur', render: (row: any) => row.ordererUser ? (
      <Link to={`/employees/${row.ordererUser.id}`} className="text-primary-700 hover:underline" onClick={(e) => e.stopPropagation()}>{row.orderer}</Link>
    ) : row.orderer },
    { key: 'agency', label: 'Agence', render: (row: any) => row.agency?.name || '-' },
    { key: 'imputation', label: 'Imputation', render: (row: any) => {
      const items: React.ReactNode[] = [];
      if (row.container) items.push(<Link key="c" to={`/containers/${row.container.id}`} className="text-primary-700 hover:underline" onClick={(e) => e.stopPropagation()}>{row.container.designation}</Link>);
      if (row.parcel) items.push(<Link key="p" to={`/parcels/${row.parcel.id}`} className="text-primary-700 hover:underline" onClick={(e) => e.stopPropagation()}>{row.parcel.trackingNumber}</Link>);
      if (row.client) items.push(<Link key="cl" to={`/clients/${row.client.id}`} className="text-primary-700 hover:underline" onClick={(e) => e.stopPropagation()}>{row.client.fullName}</Link>);
    ...
    { key: 'createdAt', label: 'Date', render: (row: any) => formatDateTime(row.createdAt) },
    {
      ...
      render: (row: any) => (
        <RowActions actions={[
          { label: 'Voir details', icon: <Eye className="h-4 w-4" />, onClick: () => navigate(`/disbursements/${row.id}`) },
          ...(canVoid ? [{ label: 'Annuler', icon: <Ban className="h-4 w-4" />, onClick: () => navigate(`/disbursements/${row.id}`), variant: 'destructive' as const, disabled: row.isVoided }] : []),
        ]} />
      ),
    },
  ];
