...
// Request interceptor: attache le JWT depuis le store auth (remplace la
// session NextAuth cote desktop).
...
async function performRefresh(): Promise<string | null> {
  ...
  // Le store gere la rotation reelle (/auth/refresh + retry doux + distinction
  // definitif/transitoire). On lit ensuite l'eventuelle erreur 'RefreshFailed'
  // pour decider du redirect.
  const newToken = await useAuthStore.getState().refresh();
  const sessionError = useAuthStore.getState().error;
