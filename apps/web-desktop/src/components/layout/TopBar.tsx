    ...
    <header className="sticky top-0 z-50 flex h-16 items-center justify-between gap-2 border-b border-gray-200 bg-white px-3 sm:px-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <AppDropdownMenu
          ...
          items={[
            { label: 'Mon profil', icon: <User className="h-4 w-4" />, onClick: () => navigate('/profile') },
            { label: 'Parametres', icon: <Settings className="h-4 w-4" />, onClick: () => navigate('/settings') },
        ...
        <div className="absolute left-0 top-full z-50 mt-2 w-full rounded-xl border border-gray-200 bg-white shadow-elevated overflow-hidden animate-fade-in">
          {!hasResults ? (
            ...
            <div className="max-h-80 overflow-y-auto">
              <ResultSection title="Colis" icon={<Package className="h-4 w-4" />} items={results.parcels} onSelect={(item) => { navigate(`/parcels/${item.id}`); setOpen(false); setQuery(''); }}
                renderItem={(item) => (
                  <div>
              ...
              />
              <ResultSection title="Clients" icon={<Users className="h-4 w-4" />} items={results.clients} onSelect={(item) => { navigate(`/clients/${item.id}`); setOpen(false); setQuery(''); }}
                renderItem={(item) => (
                  <div>
              ...
              />
              <ResultSection title="Conteneurs" icon={<Container className="h-4 w-4" />} items={results.containers} onSelect={(item) => { navigate(`/containers/${item.id}`); setOpen(false); setQuery(''); }}
                renderItem={(item) => (
                  <span className="font-mono text-sm">{item.designation}</span>
                )}
              />
              <ResultSection title="Factures" icon={<FileText className="h-4 w-4" />} items={results.invoices} onSelect={(item) => { navigate(`/invoices/${item.id}`); setOpen(false); setQuery(''); }}
                renderItem={(item) => (
